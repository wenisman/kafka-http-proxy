...
var get = function (group, id, callback) {
    ...
    createConsumerInstance = function (consumer, topic) {
        ...
        consumer.instance.on('rebalanced', function () {
            ...
            consumer.offsetMap = zookeeper.getOffsets(consumer.id, consumer.instance.topicPayloads));

            //update the kafka-node offsets
            consumer.offsetMap.forEach(function(offset){
                var item = consumer.instance.topicPayloads.filter(function(payload){
                    return payload.topic === offset.topic && 
                            payload.partition === offset.partition
                })[0];
