...
var offsets     = require('../lib/offsets');
...
var get = function (group, id, callback) {
    ...
    createConsumerInstance = function (consumer, topic) {
        ...
        consumer.instance.on('rebalanced', function () {
            ...
            consumer.offsetMap = [];
            offsets.fetchOffsets(consumer.instance.topicPayloads, function(err, data) {
                consumer.offsetMap = consumer.instance.topicPayloads.map(function(item){
                    var newItem = createOffsetItem({
                        topic: item.topic,
                        partition: item.partition,
                        offset: data[item.topic][item.partition]
                    });

                    // reset the offset within the consumer instance itself
                    item.offset = data[item.topic][item.partition];
                    return newItem;
                });

                for(var count = 0; count = consumer.instance.topicPayloads.length; count++) {
                    var topic = consumer.instance.topicPayloads[count].topic;
                    var topic = consumer.instance.topicPayloads[count].partition;
                    // recreate our own offset map
                    consumer.offsetMap.push(createOffsetItem{
                        topic: topic,
                        partition: partition,
                        offset: data[topic][partition]
                    };

                    // reset the offset within the consumer instance itself
                    consumer.instance.topicPayloads[count].offset = data[topic][partition];
                }
